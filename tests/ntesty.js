...
async function getChatbotResponse(prompt) {
    try {
        ...
        if (match) {
            ...
            const chatbotResponse = await axios.post('https://gpt4free.io/wp-json/mwai-ui/v1/chats/submit', payload, { headers });

            // Extract the relevant part of the response
            const responseData = chatbotResponse.data.split('\n\n')[chatbotResponse.data.split('\n\n').length - 2];
            return responseData;
        } else {
...
// Example usage
getChatbotResponse("How are you?").then(response => {
