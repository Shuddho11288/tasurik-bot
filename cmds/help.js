...
// Add from cmdsNew folder

const fs = require("fs");
const path = require("path");
function addFromCmdsNew() {
  fs.readdirSync(path.resolve(__dirname,"../cmdsNew")).forEach((file) => {
    if (file.endsWith(".js")) {
      const command = require(path.resolve(__dirname,`../cmdsNew/${file}`));
      helpDict[command.config.category] = helpDict[command.config.category] || {};
      helpDict[command.config.category][command.config.name] = command.config.description+"\nSyntax: "+command.config.usage+"\nAuthor: "+command.config.author;
...
function displayCommandHelp(command) {
  let cmdsNewPath = path.resolve(__dirname, "../cmdsNew");
  if (fs.existsSync(path.join(cmdsNewPath, `${command}.js`))) {
    const commando = require(path.join(cmdsNewPath, `${command}.js`));
    let obj = commando.config;
    let msg = `▢ Here is your description for your requested command ${boldify(command)} 👋\n\n`;
    for (const [key, value] of Object.entries(obj)) {
      msg += `${boldify(key.toUpperCase())}: ${value}\n`;
    }

msg += `
▢ ${boldify("NOTE")}: < > means required and [ ] means optional argument. ✅
`
    return msg;
